...
const SEGMENTS = [
  { key: "real", name: "Real", color: "#34D399" },
  { key: "waiting", name: "Espera", color: "#A78BFA" },
] as const;

export function ClickDistribution({ clicks, periodLabel }: ClickDistributionProps) {
  const counts = { real: 0, waiting: 0 };
  for (const c of clicks) {
    if (c.mode_at_click.startsWith("real")) counts.real++;
    else counts.waiting++;
  }
  const total = counts.real + counts.waiting;
